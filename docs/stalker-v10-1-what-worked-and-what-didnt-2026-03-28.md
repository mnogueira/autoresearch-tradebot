...
| Direction-specific ATR sizing on the advanced branch | `0.70x` on top-ATR days for short trades only: `R$15,618`, `PF 1.5304`, `DD 2.65%`, composite `3.7481`; `0.65x` was a near-tie at `3.7354`, `0.75x` fell to `3.7197`, all-sides trim stayed best non-directional at `3.6833`, long-only trim stayed much weaker at `3.3642`, and adding extra Thu/Fri short trims only reduced the score to `3.6240` or `3.5379` | This is now the strongest raw research-only result of the sprint. It shows the high-volatility pain is concentrated more on the short sleeve than the long sleeve, but it still depends on variable sizing rather than a static deployable preset. |
