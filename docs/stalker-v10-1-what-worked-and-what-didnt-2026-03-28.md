...
| Confidence-weighted sizing + time-widened stop | Research-only overlay on the `0.84 -> 1.20` ATR after `30` bars variant: `R$18,359.32`, `PF 1.4993`, `DD 3.72%`, composite `3.1377` | This was the earlier strongest raw research-only result before the ATR-regime sizing overlay took the lead, which still reinforces the case for future graded sizing research rather than binary filters. |
...
| ATR-regime sizing overlay on the strongest branches | Strengthened Tier 2A with `0.75x` size on top-ATR days: `R$14,808.75`, `PF 1.5239`, `DD 2.75%`, composite `3.5722`; advanced weekday-aware directional branch with the same rule: `R$15,210`, `PF 1.5371`, `DD 2.70%`, composite `3.6833` | This is now the strongest raw research-only result family of the sprint. It is the clearest sign yet that the remaining upside is in sizing and execution quality, not another entry filter, but it still depends on variable sizing instead of a static MT5 preset. |
| Fine and asymmetric ATR sizing refinement | Advanced branch finer sweep confirmed `0.75x` on top-ATR days is the local optimum (`0.60x` composite `3.6640`, `0.50x` `3.5947`, `0.85x` `3.5629`, `0.25x` `3.3836`, `0.00x` `2.9618`); adding a low-ATR boost (`1.25x` low / `0.50x` high) only reached composite `3.3666` | This tightened the ceiling assessment: the best sizing edge is simple high-volatility trimming, not a more complicated low/high asymmetry. |
| Weekday-aware sizing on the advanced branch | `0.75x` size on Thursdays and Fridays: `R$14,903.75`, `PF 1.5496`, `DD 2.72%`, composite `3.6625`; recent `60d`: `R$487.50`, `PF 1.3377`, `DD 2.43%` | This was a strong near-miss and confirms the weaker Thursday/Friday tape is real, but it still stayed just below the simpler ATR-regime sizing ceiling on the full sample. |
| ATR trim plus weekday trim on the advanced branch | Best stacked version, `0.75x` on top-ATR days plus `0.85x` on Thu/Fri, reached `R$14,367.19`, `PF 1.5496`, `DD 2.70%`, composite `3.6316` | The two de-risking ideas did not compound. That is useful ceiling evidence: the cleanest sizing edge is still just the top-ATR trim by itself. |
| Direction-specific ATR sizing on the advanced branch | `0.70x` on top-ATR days for short trades only: `R$15,618`, `PF 1.5304`, `DD 2.65%`, composite `3.7481`; `0.65x` was a near-tie at `3.7354`, `0.75x` fell to `3.7197`, all-sides trim stayed best non-directional at `3.6833`, long-only trim stayed much weaker at `3.3642`, adding extra Thu/Fri short trims only reduced the score to `3.6240` or `3.5379`, adding low-ATR long boosts only reduced it to `3.6883` or `3.6005`, and simple side-wide long/short reweighting stayed below the reference (`3.3688`, `3.3635`, `3.3143`) | This is now the strongest raw research-only result of the sprint. It shows the high-volatility pain is concentrated more on the short sleeve than the long sleeve, but it still depends on variable sizing rather than a static deployable preset. |
...
| Lower contract-range gate on Tier 2A | `FilterAsPercOfContractMARange = 0.25`: `R$14,775`, `PF 1.4944`, `DD 3.28%`, composite `3.1932` | A real full-sample lift, but the recent `60`-day tape turned negative (`R$-225`, `PF 0.8941`), so it failed the Monday-robustness bar. |
| ROC agreement window sweep on Tier 2 | `ROC(5)` stayed best over `ROC(4)`, `ROC(6)`, `ROC(20)` (`3.1117`), and `ROC(10)` (`3.0872`) on the exact composite | This confirmed the original `ROC(5)` pick was not a lucky first guess; it is still the local optimum even after the stronger Tier 2A geometry promotion. |
| Contract-cycle switch | Strengthened Tier 2A on the last `1` contract day and strengthened Tier 3 otherwise: `R$16,080`, `PF 1.5081`, `DD 3.19%`, composite `3.3578` | This became the strongest exact research line of the sprint, but it stays research-only for now because it depends on contract-cycle switching rather than one static preset. |
| Directional long/short sleeve routing | Strengthened Tier 2A longs plus strengthened Tier 3 shorts: `R$16,075`, `PF 1.5115`, `DD 3.19%`, composite `3.3433` | This was the first clear directional-asymmetry win. It also stayed positive on the `70/30` test and the recent `60`-day window, and a local sweep confirmed the original `28m` long branch plus `150m` short branch was already the true peak. It still remains research-only because it requires direction-specific sleeve routing rather than one static MT5 preset. |
| Weekday-aware directional contract-cycle sleeve routing | Full strengthened Tier 2A on the last `1` contract day and on Fridays, directional hybrid otherwise: `R$16,335`, `PF 1.5244`, `DD 3.18%`, composite `3.4017` | This is now the strongest exact research line of the sprint. It also held up on the `70/30` test and the recent `60`-day slice, but it remains research-only because it combines contract-cycle switching with weekday-aware and direction-specific sleeve routing. |
...
| Static Friday skip on strengthened Tier 2A / Tier 3 | Tier 3 skip-Friday: `R$13,040`, `PF 1.5503`, `DD 3.51%`; Tier 2A skip-Friday: `R$13,135`, `PF 1.5717`, `DD 3.57%` | Friday skipping did improve the recent `60/30/10` slices, but it weakened the full-sample composite too much to beat the plain strengthened tiers. The weekday edge seems real, but only worth using inside the more complex research routing stack. |
...
| `+DI/-DI` directional confirmation on strengthened Tier 2A | `R$15,885`, `PF 1.5117`, `DD 3.23%`, composite `3.2986` | Even a more trend-native confirmation layer still came in slightly below the strengthened Tier 2A reference, which keeps the ROC result looking specific rather than generic. |
| EMA slope confirmation on strengthened Tier 2A | `EMA(50)` slope sign was an exact tie with the strengthened Tier 2A reference; `EMA(20)` was a hair worse | A smoothed trend proxy still did not improve on `ROC(5)`, which strengthens the case that the useful agreement layer here is specific to ROC rather than any generic trend smoother. |
| CCI / Aroon confirmation on strengthened Tier 2A | `CCI` centerline: `R$15,955`, `PF 1.5139`, `DD 3.23%`, composite `3.3098`; `Aroon` dominance was worse | More trend-native proxies still came in as near-ties or degradations, which makes the `ROC(5)` edge look increasingly specific rather than a generic confirmation effect. |
| Williams %R / MFI confirmation on strengthened Tier 2A | `Williams %R` centerline: `R$15,940`, `PF 1.5134`, `DD 3.23%`, composite `3.3074`; `MFI` centerline was weaker | A price oscillator and a volume-aware oscillator both landed in the same near-tie bucket, which is more evidence that the useful agreement effect is specific to `ROC(5)`, not to generic oscillators. |
| OBV / CMF confirmation on strengthened Tier 2A | `OBV` slope sign: `R$15,150`, `PF 1.5074`, `DD 3.49%`, composite `3.0310`; `CMF` centerline: `R$14,970`, `PF 1.5030`, `DD 3.14%`, composite `3.1984` | Even the remaining volume/trend proxy corner failed to beat the strengthened Tier 2A reference, which closes off another cheap-confirmation path. |
| Ultimate Oscillator / TSI confirmation on strengthened Tier 2A | `Ultimate Oscillator` centerline: `R$14,970`, `PF 1.4853`, `DD 3.29%`, composite `3.1107`; `TSI` centerline: `R$15,940`, `PF 1.5135`, `DD 3.23%`, composite `3.3076` | The last obvious momentum/oscillator pair also came back as a degradation or near-tie, which makes `ROC(5)` look genuinely unique rather than just one member of a broad confirmation family. |
| Stochastic directional confirmation on strengthened Tier 2A | Best case `stoch_d_centerline`: `R$16,210`, `PF 1.5274`, `DD 2.98%`, composite `3.5140` | This was the first non-ROC agreement family to beat strengthened Tier 2A in-sample, but it underperformed the reference on the `70/30` test and was identical on recent `60/30/10` day windows, so it stays a ceiling note rather than a promotion. |
...
| VWAP-distance hard gates | Top-half distance: `R$9,470`, `PF 1.5206`, `DD 4.14%`; top-quartile distance: `R$5,535`, `PF 1.5557`, `DD 5.83%` | Larger deviations from session VWAP cleaned up PF a bit, but the gates over-pruned far too much and lost badly on composite. |
| VWAP-distance sizing overlay | Research-only overlay: `R$15,959.09`, `PF 1.5001`, `DD 4.03%`, composite `2.8228` | Better gross net than Tier 2A, but the added drawdown still left it below the simpler deployable ROC(5) cooldown line on full risk-adjusted score. |
| ATR high-volatility exclusion on Tier 2A | Excluding the top 33% ATR days: `R$9,800`, `PF 1.5242`, `DD 2.81%`, composite `2.6985` | Cleaner than the reference, but it gives up too much net and total composite, so it stays a niche quality mode rather than a default gate. |
| Session high/low breakout family | `SL 0.84 / TP 0.30`: `R$-7,790`, `PF 0.8607`, `DD 79.63%`; `SL 0.84 / TP 0.42`: `R$-7,270`, `PF 0.8912`, `DD 78.21%` | A genuinely different price-action family was worth testing, but it was clearly inferior to the established retracement/trend stack. |
| Session VWAP extreme mean reversion | `SL 0.84 / TP 0.30`: `R$-8,215`, `PF 0.8283`, `DD 83.81%`; `SL 0.84 / TP 0.42`: `R$-8,175`, `PF 0.8536`, `DD 83.42%` | Fading 2x ATR extensions away from session VWAP sounded like a clean opposite-regime complement, but it failed badly and did not provide a viable secondary edge. |
| Opening-range breakout family | `SL 0.84 / TP 0.30`: `R$-18,380`, `PF 0.8525`, `DD 184.52%`; `SL 0.84 / TP 0.42`: `R$-20,935`, `PF 0.8562`, `DD 210.21%` | The classic 09:00-10:00 opening-range breakout idea was even worse than the broader session-breakout prototype and is clearly not a useful alternative family here. |
| H1 timeframe translation | H1 core proxy: `R$-31,120`, `PF 0.4899`, `DD 310.91%`; H1 proxy + `3`-bar max-hold: `R$-31,280`, `PF 0.4804`, `DD 311.55%` | A slower-timeframe translation of the same retracement family is not a hidden improvement path. The edge appears to live in the faster intraday structure, not on H1. |
| Trend-strength sizing overlay on Tier 2A/Tier 3 | Best case `Tier 3` overlay: `R$18,231.63`, `PF 1.4765`, `DD 3.93%`, composite `3.0623` | Smarter sizing still raises gross net, but it increases drawdown enough that the research-only overlay does not beat the best exact deployable line on composite. |
| Simple two-tier strength sizing approximation | Best case on strengthened Tier 2A with `0.75x/1.25x` sizing: `R$16,025`, `PF 1.5098`, `DD 3.50%`, composite `3.0948` | A simpler “normal vs strong” step rule did not capture the upside of the smoother research overlays, so the sizing edge still looks graded rather than binary. |
| Practical Tier 1 + Tier 2A portfolio sleeve | Equal-weight blend: `R$16,392.50`, `PF 1.4572`, `DD 3.91%`, composite `3.0592` | Mixing the validated base sleeve with the stronger Tier 2A sleeve diluted the edge more than it smoothed risk, so it does not beat Tier 2A itself and does not change the rollout order. |
