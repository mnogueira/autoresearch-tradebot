...
- Interpretation:
  ...
  - that same sizing idea generalized to the strongest advanced branch and pushed the ceiling higher again:
    ...
    - weekday-aware sizing was another strong near-miss on that same advanced branch:
      - `0.75x` size on Thursdays and Fridays: `R$14,903.75`, `PF 1.5496`, `DD 2.72%`, composite `3.6625`
    ...
    - direction-specific ATR sizing pushed the research ceiling a bit higher again:
      - `0.70x` on top-ATR days for short trades only: `R$15,618`, `PF 1.5304`, `DD 2.65%`, composite `3.7481`
      - `0.65x` short-only trim was the next-nearest result at composite `3.7354`
      - `0.75x` short-only trim slipped to third at composite `3.7197`
  ...
  - CCI and Aroon directional confirmation also failed to beat the strengthened Tier 2A line:
...
17. Directional asymmetry is real, but it is still a research-only sleeve, not a Monday preset. The best split was strengthened Tier 2A longs plus strengthened Tier 3 shorts at `R$16,075`, `PF 1.5115`, `DD 3.19%`, composite `3.3433`; it also stayed positive on the `70/30` test (`R$3,920`, `PF 1.4424`, `DD 3.26%`) and on the recent `60`-day window (`R$450`, `PF 1.2609`, `DD 3.24%`). A local neighborhood sweep confirmed that the original `28m` long-branch cooldown and `150m` short-branch max-hold were already the true peak over nearby settings (`25/28/30m` by `120/150/180m`), and an additional ROC sweep confirmed `ROC(5)` remained the best agreement window on both sleeves over `ROC(4)` and `ROC(6)`.
18. The strongest exact research line overall is now a weekday-aware extension of the directional contract-cycle sleeve. Using full strengthened Tier 2A on the last `1` contract day and on Fridays, while keeping the directional hybrid on all other days, improved the score to `R$16,335`, `PF 1.5244`, `DD 3.18%`, composite `3.4017`; it also stayed positive on the `70/30` test (`R$3,930`, `PF 1.4436`, `DD 3.26%`) and on the recent `60`-day window (`R$480`, `PF 1.2783`, `DD 3.23%`). This extends the earlier directional contract-cycle result (`R$16,185`, composite `3.3704`) rather than replacing the Monday ladder, because it now depends on both contract-cycle switching and weekday-aware sleeve routing.
19. The directional sleeve diagnostics explain why the Friday-aware extension helps while the broader Thursday/Friday rollback does not. The directional hybrid stayed strongest on Monday through Wednesday, but recent softness clustered on Thursday and Friday. Routing only Friday back to strengthened Tier 2A lifted the research composite to `3.3793`, while routing both Thursday and Friday back to Tier 2A was slightly weaker at `3.3554`; routing Thursday alone was worse still at `3.3194`. That is useful as a ceiling map, but it is still a research-only routing stack, not a Monday deployment preset.
