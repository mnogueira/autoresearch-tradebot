...
18. The strongest exact research line overall is now a contract-cycle version of that directional sleeve. Using full strengthened Tier 2A on the last `1` contract day and the directional hybrid otherwise improved the score to `R$16,185`, `PF 1.5163`, `DD 3.18%`, composite `3.3704`; it also stayed positive on the `70/30` test (`R$3,950`, `PF 1.4458`, `DD 3.26%`) and on the recent `60`-day window (`R$480`, `PF 1.2783`, `DD 3.23%`). A local ROC sweep on the directional sleeve confirmed `ROC(5)` stayed best on both the long and short branches over `ROC(4)` and `ROC(6)`, and a recent `60`-day weekday diagnostic showed the sleeve stayed positive on Monday through Wednesday but softened on Thursday and Friday. That is useful as a ceiling marker, but it is still too operationally complex to jump into the Monday rollout path.
