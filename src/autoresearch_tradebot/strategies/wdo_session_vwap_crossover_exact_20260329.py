...
OUTPUT_DIR = artifact_output_dir("wdo_session_vwap_crossover_exact_20260329")


@dataclass(frozen=True)
class SessionVWAPCrossoverParams:
...
def build_session_vwap_crossover_signal_map(dataset: V10Dataset) -> dict[pd.Timestamp, int]:
    ...
    prev_vwap = m15["session_vwap"].shift(1)
    long_cross = (prev_close <= prev_vwap) & (m15["Close"] > m15["session_vwap"])
    short_cross = (prev_close >= prev_vwap) & (m15["Close"] < m15["session_vwap"])

    signal_map: dict[pd.Timestamp, int] = {}
    for timestamp, is_long in long_cross.items():
        if bool(is_long):
            signal_map[pd.Timestamp(timestamp) + pd.Timedelta(minutes=15)] = 1
    for timestamp, is_short in short_cross.items():
        if bool(is_short):
            signal_map[pd.Timestamp(timestamp) + pd.Timedelta(minutes=15)] = -1
    return signal_map


def run_session_vwap_crossover_backtest(
    dataset: V10Dataset,
    trade_dates: pd.Index,
    params: SessionVWAPCrossoverParams,
    ...
    signal_map = build_session_vwap_crossover_signal_map(dataset)
...
def main() -> None:
    ...
    variants = [
        ("session_vwap_crossover_sl1p0_tp0p48_cd60_skipfriday_skiplast1", SessionVWAPCrossoverParams()),
        ("session_vwap_crossover_sl1p0_tp0p60_cd60_skipfriday_skiplast1", SessionVWAPCrossoverParams(tp_atr_mult=0.60)),
    ]

    results: list[dict[str, object]] = []
    for name, params in variants:
        trades, metrics = run_session_vwap_crossover_backtest(dataset, trade_dates, params)
        results.append(
            {
                "name": name,
    ...
    summary = {
        "notes": [
            "Genuinely different corrected-cost signal family: session VWAP crossover on M15 closes.",
            "Signal uses close crossing above or below cumulative session VWAP, with next-bar exact execution and the same corrected-cost operating assumptions as the post-Monday static branches.",
        ],
        "results": results,
    }
