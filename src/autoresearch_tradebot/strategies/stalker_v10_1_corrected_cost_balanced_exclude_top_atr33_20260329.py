...
OUTPUT_DIR = artifact_output_dir("stalker_v10_1_corrected_cost_balanced_exclude_top_atr33_20260329")
...
def main() -> None:
    ...
    allowed_atr_dates = pd.Index(prior_day_atr14.index[(prior_day_atr14 <= threshold) | threshold.isna()])

    entry_filter = _combine_filters(
        session_filter({10, 11, 12, 14}),
        _date_filter(_date_set(non_last1_dates)),
        _date_filter(_date_set(allowed_atr_dates)),
    )
    ...
    payload = _variant_payload(
        name="tier2a_corrected_no_roc_no_maxhold_sl1p0_tp0p48_cd60_skipfriday_skiplast1_exclude_topatr33",
        rule=(
            "Corrected-cost balanced simplified branch with SL 1.00, TP 0.48, no ROC, no max-hold, Friday off, "
            "skip last contract day, excluding the top ATR tercile."
        ),
        trades=trades,
        trade_dates=trade_dates,
    ...
    summary = {
        ...
        "notes": [
            "Static ATR-exclusion test around the corrected-cost balanced branch.",
            "Goal: check whether simply standing down on the hottest ATR regime can preserve more edge than the baseline static branch.",
        ],
    }
