...
def _output_dir(sl_mult: float):
    ...
    return artifact_output_dir(f"stalker_v10_1_corrected_cost_survivor_no_roc_no_maxhold_sl{suffix}_20260329")
...
def main() -> None:
    ...
    payload = _variant_payload(
        name=f"tier2a_corrected_no_roc_sl{str(args.sl_mult).replace('.', 'p')}_tp0p48_cd60_skipfriday_skiplast1",
        rule=(
            "Corrected-cost strengthened Tier 2A survivor branch without ROC agreement, without max-hold, "
            f"and with SL {args.sl_mult:.2f}."
        ),
        trades=trades,
        trade_dates=trade_dates,
    ...
    summary = {
        ...
        "notes": [
            "Single-variant stop-width probe around the corrected-cost simplified holdout leader.",
            "Goal: isolate whether widening the stop improves the simpler no-ROC, no-max-hold branch.",
        ],
    }
