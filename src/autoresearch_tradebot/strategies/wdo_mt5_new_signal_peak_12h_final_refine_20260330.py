...
OUTPUT_DIR = artifact_output_dir("wdo_mt5_new_signal_peak_12h_final_refine_20260330")
...
def _signal_donchian_high_atr_12h(frame: pd.DataFrame, require_m5: bool) -> pd.Series:
...
def main() -> None:
    ...
    volume_and_spread_gate = lambda row, _: bool(pd.notna(row["vol_avg20_m1"]) and row["Volume"] > (1.5 * row["vol_avg20_m1"]) and row["spread_ticks"] <= 1.0)

    specs = [
        PivotSpec(
            name="donchian20_high_atr_12_only_tp10_volume_spread1",
            description="12h-only high-ATR Donchian with volume gate and explicit <=1 tick spread gate.",
            ...
            entry_filter_fn=volume_and_spread_gate,
        ),
        PivotSpec(
            name="donchian20_high_atr_12_only_tp10_volume_m5",
            description="12h-only high-ATR Donchian with volume gate and M5 agreement.",
            signal_fn=lambda f: _signal_donchian_high_atr_12h(f, True),
            sltp_fn=_fixed_sltp(1.0, 1.0),
            atr_col="atr14_m5",
            entry_start_hour=12,
            last_entry_hour=12,
        ...
        PivotSpec(
            name="donchian20_high_atr_12_only_tp11_volume",
            description="12h-only high-ATR Donchian with volume gate and 1.1 ATR target.",
            signal_fn=lambda f: _signal_donchian_high_atr_12h(f, False),
