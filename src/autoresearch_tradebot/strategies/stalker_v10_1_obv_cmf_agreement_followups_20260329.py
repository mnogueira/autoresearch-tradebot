...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_obv_cmf_agreement_followups_20260329")
...
def main() -> None:
    ...
    obv_slope = (obv - obv.shift(1)).shift(1).to_numpy(dtype=float)
    cmf = ChaikinMoneyFlowIndicator(high=high, low=low, close=close, volume=volume, window=20).chaikin_money_flow().shift(1).to_numpy(dtype=float)
    ...
    for name, indicator_filter in (
        ("obv_slope_sign", _make_sign_filter(obv_slope)),
        ("cmf_centerline", _make_centerline_filter(cmf, threshold=0.0)),
    ):
    ...
    if best_variant is not None:
        ...
        row["family"] = "stalker_v10_1_obv_cmf_agreement_followup"
        row["screening_method"] = "obv_cmf_agreement_followup"
