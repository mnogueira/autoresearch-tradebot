...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_advanced_atr_sizing_refine_20260329")


def _variant_payload(name: str, rule: str, trades, trade_dates, params):
...
def main() -> None:
    ...
    weights_to_test = [1.0, 0.85, 0.75, 0.6, 0.5, 0.25, 0.0]
    ranked_variants = []
    best_name = ""
    best_trades = advanced_trades

    for high_weight in weights_to_test:
        ...
        variant = _variant_payload(
            ...
            trade_dates,
            {
                ...
                "ATR_Length": 10,
                "NumDaysToConsiderPreviousContractMARange": 2,
                "atr_regime_sizing": {"lookback_days": 60, "high_weight": high_weight},
            },
        )
        ranked_variants.append(variant)
        if not best_name or float(variant["sortino_weighted_composite"]) > float(ranked_variants[0]["sortino_weighted_composite"]):
            best_name = variant["name"]
            best_trades = weighted_trades
    ...
    best_weight = ranked_variants[0]["params"]["atr_regime_sizing"]["high_weight"]
    session_weights = pd.Series(1.0, index=prior_day_atr14.index, dtype=float)
    session_weights.loc[high_mask.fillna(False)] = float(best_weight)
    best_trades = _apply_session_weights(advanced_trades, session_weights)
    ...
    update_leaderboard(DEFAULT_LEADERBOARD_PATH, [_leaderboard_row(row, summary_path) for row in ranked_variants])
