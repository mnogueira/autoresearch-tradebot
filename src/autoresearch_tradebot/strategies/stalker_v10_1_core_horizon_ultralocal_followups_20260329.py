...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_core_horizon_ultralocal_followups_20260329")
...
def main() -> None:
    ...
    variants = []
    for atr_length in [8, 9, 10, 11, 12]:
        for contract_lookback in [1, 2, 3]:
            params = replace(
                base,
                ATR_Length=atr_length,
                NumDaysToConsiderPreviousContractMARange=contract_lookback,
            )
    ...
    summary = {
        "variants": ranked,
        "notes": [
            "Ultra-local sweep around the promoted ATR10/lookback2 Tier 2A candidate.",
            "This batch is only for neighborhood checking; promotion still depends on the 70/30 and recent-tape validation pass.",
        ],
    }
