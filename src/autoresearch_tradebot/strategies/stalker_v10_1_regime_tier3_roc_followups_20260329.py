...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_regime_tier3_roc_followups_20260329")
...
def _leaderboard_row(variant: dict[str, Any], artifact: Path) -> dict[str, Any]:
    row = candidate_row(
        ...
        family="stalker_v10_1_regime_tier3_roc_followup",
    ...
    row["screening_method"] = "regime_tier3_roc_followup"
...
def main() -> None:
    ...
    trend_trades, _ = run_backtest_with_management(
        dataset=dataset,
        params=params,
        trade_dates=trade_dates,
    ...
    variant = _variant_payload(
        name="tier2_range_tier3_roc5_trend_switch",
        rule="Use Tier 2 on prior-day ADX<=25 range days and Tier 3 ROC(5)+150m max-hold on prior-day ADX>25 trend days.",
        trades=combined_trades,
        metrics=combined_metrics,
        trade_dates=trade_dates,
        params={
            "range_mode": "tier2_cooldown_25m",
            "trend_mode": "tier3_maxhold150_roc5_agreement",
            "adx_threshold": 25.0,
    ...
    train_trades = filter_trades_to_dates(combined_trades, train_dates)
    test_trades = filter_trades_to_dates(combined_trades, test_dates)
    recent_trades = filter_trades_to_dates(combined_trades, recent_dates)
    ...
    summary = {
        ...
        "notes": [
            "This batch tests a stricter regime-aware stack: plain Tier 2 in range days, Tier 3+ROC(5) in trend days.",
            "It is intended as an advanced research preset, not a Monday rollout change.",
        ],
    }
    summary_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    update_leaderboard(DEFAULT_LEADERBOARD_PATH, [_leaderboard_row(variant, summary_path)])
