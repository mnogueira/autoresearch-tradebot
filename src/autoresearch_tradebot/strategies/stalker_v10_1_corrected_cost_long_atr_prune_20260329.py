...
OUTPUT_DIR = artifact_output_dir("stalker_v10_1_corrected_cost_long_atr_prune_20260329")


def _long_atr_filter(allowed_long_dates: set[pd.Timestamp]):
    return lambda context: int(context.get("direction", 0)) == -1 or pd.Timestamp(context["session_date"]).normalize() in allowed_long_dates
...
def main() -> None:
    ...
    entry_filter = _combine_filters(
        ...
        _long_atr_filter(allowed_long_dates),
    )
    ...
    payload = _variant_payload(
        name="tier2a_corrected_no_roc_no_maxhold_sl1p0_tp0p48_cd60_skipfriday_skiplast1_prune_longs_topatr33",
        rule="Balanced corrected-cost branch that prunes only long entries on top ATR tercile days.",
        trades=trades,
        trade_dates=trade_dates,
    ...
    summary = {
        ...
        "notes": [
            "Direction-aware corrected-cost regime pruning.",
            "Goal: remove only the weak long sleeve on the hottest ATR days while keeping the stronger short sleeve active.",
        ],
    }
