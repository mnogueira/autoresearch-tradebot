...
def _output_dir(tp_mult: float):
    suffix = str(tp_mult).replace(".", "p")
    return artifact_output_dir(f"stalker_v10_1_corrected_cost_survivor_tp{suffix}_20260329")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--tp-mult", type=float, required=True)
    args = parser.parse_args()

    output_dir = _output_dir(args.tp_mult)
    ...
    payload = _variant_payload(
        name=f"tier2a_corrected_sl0p84_tp{str(args.tp_mult).replace('.', 'p')}_cd60_skipfriday_skiplast1_maxhold150",
        rule=(
            "Corrected-cost strengthened Tier 2A with 60-minute cooldown, Friday skipped, last contract day skipped, "
            f"150-minute max-hold, SL 0.84, and TP {args.tp_mult:.2f}."
        ),
        trades=trades,
        trade_dates=trade_dates,
    ...
    summary = {
        ...
        "notes": [
            "Single-variant take-profit probe around the best corrected-cost static survivor.",
            "Goal: isolate whether pushing the survivor target beyond 0.48 can improve cost-adjusted returns without reintroducing overtrading.",
        ],
    }
