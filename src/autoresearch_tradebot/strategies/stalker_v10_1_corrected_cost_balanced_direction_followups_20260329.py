...
OUTPUT_DIR = artifact_output_dir("stalker_v10_1_corrected_cost_balanced_direction_followups_20260329")
...
def main() -> None:
    ...
    variants = []
    for name, wanted_direction in (("long_only", 1), ("short_only", -1)):
        def _direction_filter(context, wanted: int = wanted_direction) -> bool:
            return int(context.get("direction", 0)) == wanted

        entry_filter = _combine_filters(base_filter, _direction_filter)
        ...
        payload = _variant_payload(
            name=f"tier2a_corrected_no_roc_no_maxhold_sl1p0_tp0p48_cd60_skipfriday_skiplast1_{name}",
            rule=f"Balanced corrected-cost branch using {name.replace('_', ' ')}.",
            trades=trades,
            trade_dates=trade_dates,
            params={"params": asdict(params), "management": asdict(management), "direction": name},
        )
        variants.append(
            {
                "variant": payload,
    ...
    (OUTPUT_DIR / "summary.json").write_text(json.dumps({"variants": variants}, indent=2), encoding="utf-8")
