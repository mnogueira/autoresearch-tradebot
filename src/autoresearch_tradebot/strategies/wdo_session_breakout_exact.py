...
DEFAULT_OUTPUT_DIR = artifact_output_dir("wdo_session_breakout_exact_20260329")
...
def build_session_breakout_signal_map(bars: pd.DataFrame) -> dict[pd.Timestamp, int]:
    m15 = (
    ...
    prev_session_high = m15.groupby("session_date_norm")["High"].cummax().shift(1)
    prev_session_low = m15.groupby("session_date_norm")["Low"].cummin().shift(1)
    long_breakout = m15["Close"] > prev_session_high
    short_breakout = m15["Close"] < prev_session_low
...
def run_session_breakout_backtest(
    dataset: V10Dataset,
    trade_dates: pd.Index,
    params: SessionBreakoutParams,
    ...
    signal_map = build_session_breakout_signal_map(dataset.bars_m1)
...
def candidate_row(name: str, params: SessionBreakoutParams, metrics: dict[str, object], summary_path: Path) -> dict[str, object]:
    return {
        "name": name,
        "family": "session_breakout_exact",
        "screening_method": "exact_every_tick_breakout_entry",
        ...
        "notes": "Exact session high/low breakout entered on the next M1 open with the same session filter and cooldown as the current production family.",
...
def main() -> None:
    ...
    candidates = [
        ("session_breakout_exact_sl0p84_tp0p30_cd25", SessionBreakoutParams(sl_atr_mult=0.84, tp_atr_mult=0.30)),
        ("session_breakout_exact_sl0p84_tp0p42_cd25", SessionBreakoutParams(sl_atr_mult=0.84, tp_atr_mult=0.42)),
    ]

    results: list[dict[str, object]] = []
    leaderboard_rows: list[dict[str, object]] = []
    for name, params in candidates:
        _, metrics = run_session_breakout_backtest(dataset, dataset.trade_dates, params)
        results.append({"name": name, "params": params.__dict__, "metrics": metrics})

    summary = {
        "notes": [
            "Exact session breakout entered at the next M1 open after an M15 close makes a new session high or low.",
