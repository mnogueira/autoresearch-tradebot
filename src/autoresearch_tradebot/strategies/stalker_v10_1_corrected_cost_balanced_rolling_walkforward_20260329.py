...
OUTPUT_DIR = artifact_output_dir("stalker_v10_1_corrected_cost_balanced_rolling_walkforward_20260329")
...
def _build_trade_dates(dataset: V10Dataset) -> pd.Index:
    ...
    allowed_dates = pd.Index(trade_dates).intersection(non_last1_dates)
    return allowed_dates.sort_values()
...
def main() -> None:
    ...
    summary = {
        "variant": _variant_payload(
            name="tier2a_corrected_no_roc_no_maxhold_sl1p0_tp0p48_cd60_skipfriday_skiplast1",
            rule="Balanced corrected-cost static branch used for rolling walk-forward validation.",
            trades=full_trades,
            trade_dates=trade_dates,
        ...
        "notes": [
            "Anchored-corrected-cost branch was already positive on a single 70/30 split.",
            "This rolling walk-forward checks whether that positive behavior repeats across multiple chronological windows.",
        ],
    }
