...
OUTPUT_DIR = artifact_output_dir("stalker_v10_1_corrected_cost_combo_skip14_20260329")
...
def main() -> None:
    ...
    entry_filter = _combine_filters(
        session_filter({10, 11, 12, 13}),
        _date_filter(_date_set(non_last1_dates)),
        _directional_filter(allowed_long_dates, {10, 11, 12, 13}),
    )
    ...
    summary = {
        "variant": _variant_payload(
            name="tier2a_corrected_no_roc_no_maxhold_sl1p0_tp0p48_cd60_skipfriday_skiplast1_prune_longs_topatr33_skip14",
            rule="Corrected-cost stacked combo with 14h removed for both directions.",
            trades=trades,
            trade_dates=trade_dates,
        ...
        "notes": [
            "Corrected-cost stacked combo with 14h removed entirely.",
            "Follow-up to the hour-by-hour PnL map showing 14h as the only negative hour on the combo leader.",
        ],
    }
