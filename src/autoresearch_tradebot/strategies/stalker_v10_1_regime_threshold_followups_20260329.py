...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_regime_threshold_followups_20260329")
...
def _leaderboard_row(variant: dict[str, Any], artifact: Path) -> dict[str, Any]:
    row = candidate_row(
        ...
        family="stalker_v10_1_regime_threshold_followup",
    ...
    row["screening_method"] = "regime_threshold_followup"
...
def main() -> None:
    ...
    for threshold in (20.0, 22.5, 25.0, 27.5, 30.0):
        trend_dates, range_dates, _ = split_trade_dates_by_adx(dataset, threshold=threshold)
        ...
        name = f"tier2_range_tier3_roc5_trend_switch_adx_{str(threshold).replace('.', 'p')}"
        variants.append(
            _variant_payload(
                name=name,
                rule=f"Use Tier 2 on prior-day ADX<={threshold:g} range days and Tier 3 ROC(5)+150m max-hold on prior-day ADX>{threshold:g} trend days.",
    ...
    summary = {
        ...
        "notes": [
            "This batch refines the prior-day ADX threshold inside the strongest regime-aware ROC stack.",
            "Range days use plain Tier 2; trend days use Tier 3 plus ROC(5) agreement.",
        ],
    }
