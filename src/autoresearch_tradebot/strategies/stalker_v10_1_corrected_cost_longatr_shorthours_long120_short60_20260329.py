...
OUTPUT_DIR = artifact_output_dir(
    "stalker_v10_1_corrected_cost_longatr_shorthours_long120_short60_20260329"
)
...
def _run_long_branch(dataset: V10Dataset, trade_dates: pd.Index) -> pd.DataFrame:
    entry_filter = _combine_filters(
        session_filter({10, 11, 12, 14}),
        _date_filter(_date_set(_non_last1_dates(dataset))),
    ...
    trades, _ = run_backtest_with_management(
        ...
        management=ManagementConfig(min_minutes_between_entries=120),
    )
    return trades


def _run_short_branch(dataset: V10Dataset, trade_dates: pd.Index) -> pd.DataFrame:
    entry_filter = _combine_filters(
...
def main() -> None:
    ...
    trades = _combined_trades(
        _run_long_branch(dataset, trade_dates),
        _run_short_branch(dataset, trade_dates),
    )

    payload = _variant_payload(
        name="tier2a_corrected_no_roc_no_maxhold_sl1p0_tp0p48_longatrprune_long120_shorts101112_short60",
        rule="Corrected-cost side-specific routing with long ATR prune, long cooldown 120m, and shorts restricted to 10/11/12 at 60m cooldown.",
        trades=trades,
        trade_dates=trade_dates,
        params={
            "long_cooldown_minutes": 120,
            "short_cooldown_minutes": 60,
            "long_atr_prune": "exclude top ATR tercile",
            "short_hours": [10, 11, 12],
            "params": asdict(_common_params()),
        },
    )
    ...
    summary = {
        ...
        "notes": [
            "Corrected-cost stacked side-specific routing check.",
            "Goal: keep the short sleeve at 60m while slowing only the weaker long sleeve to 120m on top of the current combo leader.",
        ],
    }
