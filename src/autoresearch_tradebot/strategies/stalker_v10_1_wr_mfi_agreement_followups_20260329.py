...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_wr_mfi_agreement_followups_20260329")
...
def _make_centerline_filter(array, threshold: float):
    def allow(context):
...
def main() -> None:
    ...
    williams_r = WilliamsRIndicator(high=high, low=low, close=close, lbp=14).williams_r().shift(1).to_numpy(dtype=float)
    mfi = MFIIndicator(high=high, low=low, close=close, volume=volume, window=14).money_flow_index().shift(1).to_numpy(dtype=float)
    ...
    for name, indicator_filter in (
        ("williams_r_centerline", _make_centerline_filter(williams_r, threshold=-50.0)),
        ("mfi_centerline", _make_centerline_filter(mfi, threshold=50.0)),
    ):
    ...
    if best_variant is not None:
        ...
        row["family"] = "stalker_v10_1_wr_mfi_agreement_followup"
        row["screening_method"] = "wr_mfi_agreement_followup"
