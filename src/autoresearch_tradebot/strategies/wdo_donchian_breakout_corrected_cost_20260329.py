...
OUTPUT_DIR = artifact_output_dir("wdo_donchian_breakout_corrected_cost_20260329")


@dataclass(frozen=True)
class DonchianCorrectedParams:
    lookback_bars: int
...
def build_donchian_signal_map(dataset: V10Dataset, params: DonchianCorrectedParams) -> dict[pd.Timestamp, int]:
    m15 = (
        dataset.bars_m1.resample("15min")
        .agg({"Open": "first", "High": "max", "Low": "min", "Close": "last"})
        .dropna()
    )
    upper = m15["High"].rolling(params.lookback_bars, min_periods=params.lookback_bars).max().shift(1)
    lower = m15["Low"].rolling(params.lookback_bars, min_periods=params.lookback_bars).min().shift(1)
    long_break = m15["Close"] > upper
    short_break = m15["Close"] < lower

    signal_map: dict[pd.Timestamp, int] = {}
    for timestamp, is_long in long_break.items():
        if bool(is_long):
            signal_map[pd.Timestamp(timestamp) + pd.Timedelta(minutes=15)] = 1
    for timestamp, is_short in short_break.items():
        if bool(is_short):
            signal_map[pd.Timestamp(timestamp) + pd.Timedelta(minutes=15)] = -1
    return signal_map
...
def run_donchian_backtest(
    ...
    params: DonchianCorrectedParams,
    ...
    signal_map = build_donchian_signal_map(dataset, params)
...
def main() -> None:
    ...
    variants = [
        ("donchian_breakout_n10_sl1p0_tp0p48_cd60_skipfriday_skiplast1", DonchianCorrectedParams(lookback_bars=10)),
        ("donchian_breakout_n20_sl1p0_tp0p48_cd60_skipfriday_skiplast1", DonchianCorrectedParams(lookback_bars=20)),
        ("donchian_breakout_n30_sl1p0_tp0p48_cd60_skipfriday_skiplast1", DonchianCorrectedParams(lookback_bars=30)),
    ]

    results: list[dict[str, object]] = []
    for name, params in variants:
        trades, metrics = run_donchian_backtest(dataset, trade_dates, params)
        results.append(
            {
                "name": name,
    ...
    summary = {
        "notes": [
            "Corrected-cost Donchian breakout scout on M15 closes with next-bar exact execution.",
            "Lookbacks 10, 20, and 30 were chosen to test whether a cleaner trend-following family can survive costs better than the retracement-based combo.",
        ],
        "results": results,
    }
