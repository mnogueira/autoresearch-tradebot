...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_advanced_directional_atr_sizing_followups_20260329")


def _apply_directional_session_weights(
    trades: pd.DataFrame,
    session_weights: pd.Series,
    target_direction: str | None,
) -> pd.DataFrame:
    if trades.empty:
        return trades.copy()
    frame = trades.copy()
    base_weights = pd.to_datetime(frame["session_date"]).dt.normalize().map(session_weights).fillna(1.0).astype(float)
    if target_direction is None:
        ...
        weights.loc[direction_mask] = base_weights.loc[direction_mask]
    frame["directional_atr_size_multiplier"] = weights.to_numpy(dtype=float)
    frame["pnl_brl"] = frame["pnl_brl"].astype(float) * frame["directional_atr_size_multiplier"]
    if "pnl_points" in frame.columns:
        frame["pnl_points"] = frame["pnl_points"].astype(float) * frame["directional_atr_size_multiplier"]
    return frame
...
def main() -> None:
    ...
    session_weights.loc[(prior_day_atr14 > q67).fillna(False)] = 0.75
    ...
    variants = [
        ...
        _variant_payload(
            "advanced_high_atr_all_0p75",
            "Advanced branch with 0.75x size on top-ATR days for both directions.",
            _apply_directional_session_weights(advanced_trades, session_weights, None),
            trade_dates,
            {"directional_atr_target": "both", "high_weight": 0.75},
        ),
        _variant_payload(
            "advanced_high_atr_long_0p75",
            "Advanced branch with 0.75x size on top-ATR days for long trades only.",
            _apply_directional_session_weights(advanced_trades, session_weights, "long"),
            trade_dates,
            {"directional_atr_target": "long", "high_weight": 0.75},
        ),
        _variant_payload(
            "advanced_high_atr_short_0p75",
            "Advanced branch with 0.75x size on top-ATR days for short trades only.",
            _apply_directional_session_weights(advanced_trades, session_weights, "short"),
            trade_dates,
            {"directional_atr_target": "short", "high_weight": 0.75},
        ),
    ]
    ...
    best = ranked[0]
    best_direction = best["params"].get("directional_atr_target")
    best_direction = None if best_direction == "both" or best_direction is None else str(best_direction)
    best_trades = advanced_trades if best["name"] == "advanced_reference" else _apply_directional_session_weights(
        advanced_trades, session_weights, best_direction
    )
    ...
    summary = {
        ...
        "notes": [
            "Research-only directional ATR sizing overlay on the strongest advanced weekday-aware directional branch.",
            "Purpose: check whether the high-ATR trim belongs mostly on the long sleeve, the short sleeve, or both.",
        ],
    }
