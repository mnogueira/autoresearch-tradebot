...
OUTPUT_DIR = artifact_output_dir("wdo_momentum_ignition_corrected_cost_20260329")


@dataclass(frozen=True)
class MomentumIgnitionParams:
    return_atr_mult: float = 0.75
    volume_mult: float = 1.5
    close_location_frac: float = 0.75
...
def build_momentum_ignition_signal_map(dataset: V10Dataset, params: MomentumIgnitionParams) -> dict[pd.Timestamp, int]:
    m15 = (
        dataset.bars_m1.resample("15min")
    ...
    tr = pd.concat(
        [
            (m15["High"] - m15["Low"]).abs(),
            (m15["High"] - prev_close).abs(),
            (m15["Low"] - prev_close).abs(),
        ],
        axis=1,
    ).max(axis=1)
    atr = tr.rolling(params.atr_length, min_periods=params.atr_length).mean()
    volume_avg = m15["Volume"].rolling(20, min_periods=10).mean()

    bar_return = m15["Close"] - m15["Open"]
    bar_range = (m15["High"] - m15["Low"]).replace(0.0, np.nan)
    close_loc = (m15["Close"] - m15["Low"]) / bar_range

    long_fire = (
        (bar_return >= (params.return_atr_mult * atr))
        & (m15["Volume"] >= (params.volume_mult * volume_avg))
        & (close_loc >= params.close_location_frac)
    )
    short_fire = (
        (-bar_return >= (params.return_atr_mult * atr))
        & (m15["Volume"] >= (params.volume_mult * volume_avg))
        & (close_loc <= (1.0 - params.close_location_frac))
    )

    signal_map: dict[pd.Timestamp, int] = {}
    for timestamp, is_long in long_fire.items():
        if bool(is_long):
            signal_map[pd.Timestamp(timestamp) + pd.Timedelta(minutes=15)] = 1
    for timestamp, is_short in short_fire.items():
        if bool(is_short):
            signal_map[pd.Timestamp(timestamp) + pd.Timedelta(minutes=15)] = -1
    return signal_map
...
def run_momentum_ignition_backtest(
    ...
    signal_map = build_momentum_ignition_signal_map(dataset, params)
...
def main() -> None:
    ...
    variants = [
        ("momentum_ignition_ret0p75atr_vol1p5_close0p75_tp0p48", MomentumIgnitionParams()),
        ("momentum_ignition_ret0p75atr_vol1p5_close0p75_tp0p60", MomentumIgnitionParams(tp_atr_mult=0.60)),
    ]

    results: list[dict[str, object]] = []
    for name, params in variants:
        trades, metrics = run_momentum_ignition_backtest(dataset, trade_dates, params)
        results.append(
            {
                "name": name,
    ...
    summary = {
        "notes": [
            "Genuinely new corrected-cost signal family: M15 momentum ignition continuation.",
            "Requires a large directional M15 bar versus ATR, a 1.5x volume spike, and a close near the bar extreme before entering on the next bar.",
        ],
        "results": results,
    }
