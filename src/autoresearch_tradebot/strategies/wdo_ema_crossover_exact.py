...
DEFAULT_OUTPUT_DIR = artifact_output_dir("wdo_ema_crossover_exact_20260328")
...
def build_m15_crossover_signal_map(bars: pd.DataFrame, fast_span: int, slow_span: int) -> dict[pd.Timestamp, int]:
    m15_close = bars["Close"].resample("15min").last()
    ema_fast = m15_close.ewm(span=int(fast_span), adjust=False).mean()
    ema_slow = m15_close.ewm(span=int(slow_span), adjust=False).mean()
    cross_up = (ema_fast > ema_slow) & (ema_fast.shift(1) <= ema_slow.shift(1))
    cross_down = (ema_fast < ema_slow) & (ema_fast.shift(1) >= ema_slow.shift(1))

    signal_map: dict[pd.Timestamp, int] = {}
    for timestamp, is_up in cross_up.items():
        if bool(is_up):
            signal_map[pd.Timestamp(timestamp) + pd.Timedelta(minutes=15)] = 1
    for timestamp, is_down in cross_down.items():
        if bool(is_down):
            signal_map[pd.Timestamp(timestamp) + pd.Timedelta(minutes=15)] = -1
    return signal_map
...
def run_ema_crossover_backtest(
    ...
    signal_map = build_m15_crossover_signal_map(dataset.bars_m1, params.fast_span, params.slow_span)
    ...
    for index in range(len(timestamps)):
        ...
        if position == 0 and signal_direction in (-1, 1) and allowed_time and np.isfinite(atr_value) and atr_value > 0.0:
...
def candidate_row(name: str, params: EMACrossParams, metrics: dict[str, Any], summary_path: Path) -> dict[str, Any]:
    return {
        "name": name,
        "family": "ema_crossover_exact",
        "screening_method": "exact_every_tick_market_entry",
        ...
        "notes": "Exact every-tick EMA crossover market-entry variant with session filter.",
...
def main() -> None:
    ...
    candidates = [
        ("ema_5_21_exact_sl0p84_tp0p42", EMACrossParams(fast_span=5, slow_span=21, sl_atr_mult=0.84, tp_atr_mult=0.42)),
        ("ema_5_21_exact_sl0p84_tp0p30", EMACrossParams(fast_span=5, slow_span=21, sl_atr_mult=0.84, tp_atr_mult=0.30)),
        ("ema_8_34_exact_sl0p84_tp0p42", EMACrossParams(fast_span=8, slow_span=34, sl_atr_mult=0.84, tp_atr_mult=0.42)),
        ("ema_3_13_exact_sl0p84_tp0p42", EMACrossParams(fast_span=3, slow_span=13, sl_atr_mult=0.84, tp_atr_mult=0.42)),
    ]

    results: list[dict[str, Any]] = []
    leaderboard_rows: list[dict[str, Any]] = []
    for name, params in candidates:
        trades_df, metrics = run_ema_crossover_backtest(dataset, dataset.trade_dates, params)
        year_rows = []
        frame = trades_df.copy()
        if not frame.empty:
            frame["session_date"] = pd.to_datetime(frame["session_date"]).dt.normalize()
        for year in (2024, 2025):
            year_dates = pd.Index([date for date in dataset.trade_dates if pd.Timestamp(date).year == year])
            year_df = frame[frame["session_date"].dt.year.eq(year)] if not frame.empty else frame
            if len(year_dates):
                year_rows.append({"year": year, "metrics": calculate_metrics(year_df, year_dates)})
        results.append(
            {
                "name": name,
                "params": {
    ...
    summary = {
        "notes": [
            "Exact every-tick engine using market entries at the next M1 open after an M15 EMA crossover event.",
            "This is a different strategy family from the v10.1 retracement system.",
        ],
        "results": results,
    }
