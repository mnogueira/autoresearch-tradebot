...
OUTPUT_DIR = artifact_output_dir("stalker_v10_1_corrected_cost_short_only_sl_probe_20260329")
...
def main() -> None:
    ...
    for sl_multiplier in (1.0, 1.2, 1.5):
        params = replace(base_params, SL_ATRMultiplier=sl_multiplier)
        ...
        payload = _variant_payload(
            name=f"tier2a_corrected_no_roc_no_maxhold_sl{str(sl_multiplier).replace('.', 'p')}_tp0p48_cd60_skipfriday_skiplast1_short_only",
            rule=f"Short-only corrected-cost branch with SL {sl_multiplier:.2f}.",
            trades=trades,
            trade_dates=trade_dates,
            params={
    ...
    summary = {
        ...
        "notes": [
            "Short-side corrected-cost SL sweep after the direction split showed shorts carry most of the robust edge.",
            "Goal: test whether the short sleeve wants more breathing room than the balanced corrected-cost branch.",
        ],
    }
