...
from ta.momentum import TSIIndicator, UltimateOscillator
...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_uo_tsi_agreement_followups_20260329")
...
def main() -> None:
    ...
    ultimate = UltimateOscillator(high=high, low=low, close=close).ultimate_oscillator().shift(1).to_numpy(dtype=float)
    tsi = TSIIndicator(close=close, window_slow=25, window_fast=13).tsi().shift(1).to_numpy(dtype=float)
    ...
    for name, indicator_filter in (
        ("ultimate_oscillator_centerline", _make_centerline_filter(ultimate, threshold=50.0)),
        ("tsi_centerline", _make_centerline_filter(tsi, threshold=0.0)),
    ):
    ...
    if best_variant is not None:
        ...
        row["family"] = "stalker_v10_1_uo_tsi_agreement_followup"
        row["screening_method"] = "uo_tsi_agreement_followup"
