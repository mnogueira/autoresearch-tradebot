...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_advanced_weekday_sizing_followups_20260329")
...
def main() -> None:
    ...
    weekday_weights = [
        ("advanced_weekday_directional_reference", "Advanced weekday-aware directional branch reference.", {}, 1.0),
        ("advanced_weekday_directional_thu_fri_0p75", "Advanced branch with 0.75x size on Thursdays and Fridays.", {3, 4}, 0.75),
        ("advanced_weekday_directional_thu_fri_0p5", "Advanced branch with 0.50x size on Thursdays and Fridays.", {3, 4}, 0.50),
        ("advanced_weekday_directional_thu_0p75", "Advanced branch with 0.75x size on Thursdays only.", {3}, 0.75),
        ("advanced_weekday_directional_fri_0p75", "Advanced branch with 0.75x size on Fridays only.", {4}, 0.75),
    ]

    ranked = []
    for name, rule, weekdays, weight in weekday_weights:
        session_weights = pd.Series(1.0, index=trade_dates, dtype=float)
        if weekdays:
            target_dates = _weekday_date_index(trade_dates, weekdays)
            session_weights.loc[target_dates] = float(weight)
    ...
    weekdays = set(best["params"]["weekday_sizing"]["weekdays"])
    if weekdays:
        best_weights.loc[_weekday_date_index(trade_dates, weekdays)] = float(best["params"]["weekday_sizing"]["weight"])
    ...
    summary = {
        ...
        "notes": [
            "Research-only weekday sizing overlay on the strongest advanced weekday-aware directional branch.",
            "Purpose: check whether softer Thursday/Friday tapes should be sized down instead of filtered.",
        ],
    }
    ...
    update_leaderboard(
        ...
        [_leaderboard_row(variant, summary_path) for variant in ranked if variant["name"] != "advanced_weekday_directional_reference"],
    )
