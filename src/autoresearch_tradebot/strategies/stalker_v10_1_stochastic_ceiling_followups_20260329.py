...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_stochastic_ceiling_followups_20260329")
...
def main() -> None:
    ...
    stoch_k = indicator.stoch().shift(1).to_numpy(dtype=float)
    stoch_d = indicator.stoch_signal().shift(1).to_numpy(dtype=float)
    stoch_gap = (indicator.stoch() - indicator.stoch_signal()).shift(1).to_numpy(dtype=float)
    ...
    for name, indicator_filter in (
        ("stoch_k_centerline", _make_centerline_filter(stoch_k)),
        ("stoch_gap_sign", _make_sign_filter(stoch_gap)),
        ("stoch_d_centerline", _make_centerline_filter(stoch_d)),
    ):
        ...
        variant = {
            ...
            **_risk_block(trades, trade_dates),
            "params": {
                "stochastic_variant": name,
                "stochastic_window": 14,
                "stochastic_smooth_window": 3,
    ...
    if best_variant is not None:
        ...
        row["family"] = "stalker_v10_1_stochastic_ceiling_followup"
        row["screening_method"] = "stochastic_ceiling_followup"
