...
OUTPUT_DIR = artifact_output_dir("stalker_v10_1_corrected_cost_short_only_hour_probe_20260329")
...
def main() -> None:
    ...
    for hours in ({10, 11}, {10, 11, 12}, {10, 11, 12, 14}):
        entry_filter = _combine_filters(
            session_filter(set(hours)),
        ...
        hour_label = "_".join(str(hour) for hour in sorted(hours))
        payload = _variant_payload(
            name=f"tier2a_corrected_no_roc_no_maxhold_sl1p0_tp0p48_cd60_skipfriday_skiplast1_short_only_hours_{hour_label}",
            rule=f"Short-only corrected-cost branch restricted to hours {sorted(hours)}.",
            trades=trades,
            trade_dates=trade_dates,
            params={
    ...
    summary = {
        ...
        "notes": [
            "Short-only corrected-cost session-hour probe.",
            "Goal: check whether the honest short-side edge is concentrated in the first part of the session.",
        ],
    }
