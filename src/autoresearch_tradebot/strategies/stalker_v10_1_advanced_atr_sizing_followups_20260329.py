...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_advanced_atr_sizing_followups_20260329")
...
def main() -> None:
    ...
    variants = [
        _variant_payload(
            ...
            "Strengthened Tier 2A on last 1 contract day and Fridays, directional hybrid otherwise.",
            advanced_trades,
            trade_dates,
            {
    ...
    for high_weight in (0.75, 0.5):
        ...
        variants.append(
            _variant_payload(
                f"advanced_weekday_directional_high_atr_{str(high_weight).replace('.', 'p')}",
                f"Advanced weekday-aware directional branch with {high_weight}x size on top-ATR days.",
                weighted_trades,
                trade_dates,
                {
                    "tier2a_management": asdict(tier2a_management),
    ...
    best = ranked[0]
    best_trades = advanced_trades if best["name"] == "advanced_weekday_directional_reference" else _apply_session_weights(
        advanced_trades,
        pd.Series(1.0, index=prior_day_atr14.index, dtype=float).where(~high_mask.fillna(False), float(best["params"]["atr_regime_sizing"]["high_weight"])),
    )
    ...
    summary = {
        ...
        "notes": [
            "Research-only sizing overlay on the strongest advanced weekday-aware directional branch.",
            "Purpose: test whether the new ATR-regime sizing edge generalizes beyond strengthened Tier 2A.",
        ],
    }
    ...
    update_leaderboard(
        ...
        [_leaderboard_row(variant, summary_path) for variant in variants if variant["name"] != "advanced_weekday_directional_reference"],
    )
