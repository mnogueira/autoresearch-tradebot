...
DEFAULT_OUTPUT_DIR = artifact_output_dir("wdo_inside_bar_breakout_exact_20260328")
...
def build_inside_bar_signal_map(bars: pd.DataFrame) -> dict[pd.Timestamp, int]:
    m15 = bars.resample("15min").agg({"Open": "first", "High": "max", "Low": "min", "Close": "last"}).dropna()
    inside = (m15["High"] < m15["High"].shift(1)) & (m15["Low"] > m15["Low"].shift(1))
    long_breakout = inside.shift(1).fillna(False) & (m15["Close"] > m15["High"].shift(1))
    short_breakout = inside.shift(1).fillna(False) & (m15["Close"] < m15["Low"].shift(1))
...
def run_inside_bar_backtest(dataset: V10Dataset, trade_dates: pd.Index, params: InsideBarParams) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    signal_map = build_inside_bar_signal_map(dataset.bars_m1)
...
def candidate_row(name: str, params: InsideBarParams, metrics: dict[str, Any], summary_path: Path) -> dict[str, Any]:
    return {
        "name": name,
        "family": "inside_bar_breakout_exact",
        ...
        "notes": "Exact M15 inside-bar breakout, entered on next M1 open, with session filter.",
...
def main() -> None:
    ...
    candidates = [
        ("inside_bar_breakout_exact_sl0p84_tp0p30", InsideBarParams(sl_atr_mult=0.84, tp_atr_mult=0.30)),
        ("inside_bar_breakout_exact_sl0p84_tp0p48", InsideBarParams(sl_atr_mult=0.84, tp_atr_mult=0.48)),
    ]

    results: list[dict[str, Any]] = []
    leaderboard_rows: list[dict[str, Any]] = []
    for name, params in candidates:
        _, metrics = run_inside_bar_backtest(dataset, dataset.trade_dates, params)
        results.append({"name": name, "params": params.__dict__, "metrics": metrics})

    summary = {
        "notes": [
            "Exact M15 inside-bar breakout entered at the next M1 open after a confirmed breakout close.",
            "Session filter kept to 10:00, 11:00, 12:00, and 14:00 to stay comparable with the session winner.",
        ],
        "results": results,
    }
