...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_macd_agreement_followups_20260329")
...
def _make_directional_filter(array, *, nonnegative: bool):
    def allow(context):
        ...
        if direction == 1:
            return value >= 0.0 if nonnegative else value > 0.0
        return value <= 0.0 if nonnegative else value < 0.0
...
def main() -> None:
    ...
    macd_line = macd_indicator.macd().shift(1).to_numpy(dtype=float)
    macd_hist = macd_indicator.macd_diff().shift(1).to_numpy(dtype=float)
    ...
    for name, indicator_filter in (
        ("macd_line_sign", _make_directional_filter(macd_line, nonnegative=True)),
        ("macd_hist_sign", _make_directional_filter(macd_hist, nonnegative=False)),
    ):
    ...
    if best_variant is not None:
        ...
        row["family"] = "stalker_v10_1_macd_agreement_followup"
        row["screening_method"] = "macd_agreement_followup"
