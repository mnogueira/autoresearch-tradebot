...
def _output_dir(tp_mult: float):
    ...
    return artifact_output_dir(f"stalker_v10_1_corrected_cost_survivor_no_roc_no_maxhold_tp{suffix}_20260329")
...
def main() -> None:
    ...
    payload = _variant_payload(
        name=f"tier2a_corrected_no_roc_sl1p0_tp{str(args.tp_mult).replace('.', 'p')}_cd60_skipfriday_skiplast1",
        rule=(
            "Corrected-cost strengthened Tier 2A simplified survivor branch with SL 1.00, no ROC, no max-hold, "
            f"and TP {args.tp_mult:.2f}."
        ),
        trades=trades,
        trade_dates=trade_dates,
    ...
    summary = {
        ...
        "notes": [
            "Single-variant take-profit probe around the corrected-cost balanced leader.",
            "Goal: isolate whether the simplified SL 1.00 branch can support a wider take-profit.",
        ],
    }
