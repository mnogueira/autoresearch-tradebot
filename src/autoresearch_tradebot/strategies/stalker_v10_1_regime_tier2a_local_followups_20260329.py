...
DEFAULT_OUTPUT_DIR = artifact_output_dir("stalker_v10_1_regime_tier2a_local_followups_20260329")
...
def _leaderboard_row(variant: dict[str, Any], artifact: Path) -> dict[str, Any]:
    row = candidate_row(
        ...
        family="stalker_v10_1_regime_tier2a_local_followup",
    ...
    row["screening_method"] = "regime_tier2a_local_followup"
...
def main() -> None:
    ...
    local_params = replace(base_params, ATR_Length=10, NumDaysToConsiderPreviousContractMARange=2)
    ...
    tier2a_local_trades, tier2a_local_metrics = run_backtest_with_management(
        dataset=dataset,
        params=local_params,
        trade_dates=trade_dates,
    ...
    trend_tier2a_local_trades, _ = run_backtest_with_management(
        dataset=dataset,
        params=local_params,
        trade_dates=trade_dates,
    ...
    regime_switch_trades, regime_switch_metrics = combine_runs(
        [range_tier2_trades, trend_tier2a_local_trades],
        trade_dates,
    )
    ...
    variants = [
        ...
        _payload(
            "tier2a_local_reference",
            "Current strongest Tier 2A upgrade: ROC(5) agreement plus ATR_Length 10 and contract lookback 2.",
            tier2a_local_trades,
            tier2a_local_metrics,
            trade_dates,
            {
        ...
        _payload(
            "tier2_range_tier2a_local_trend_switch",
            "Use Tier 2 on prior-day ADX<=25 range days and the stronger Tier 2A local horizon variant on prior-day ADX>25 trend days.",
            ...
            trade_dates,
            {
                "management": asdict(management),
                "range_mode": "tier2",
                "trend_mode": "tier2a_local_reference",
                "adx_threshold": 25.0,
                "roc_agreement_bars": 5,
                "ATR_Length": 10,
                "NumDaysToConsiderPreviousContractMARange": 2,
    ...
    trade_map = {
        ...
        "tier2a_local_reference": tier2a_local_trades,
        "tier2_range_tier2a_local_trend_switch": regime_switch_trades,
    }
    ...
    summary = {
        ...
        "notes": [
            "This batch checks whether the stronger ATR10/lookback2 ROC upgrade should only be used in trend days while plain Tier 2 handles range days.",
            "If the regime switch is only a tie or worse, the simpler always-on Tier 2A local variant is the better operational choice.",
        ],
    }
